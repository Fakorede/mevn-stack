...
export default {
  async asyncData({ $axios }) {
    try {
      let { addresses } = await $axios.$get("/api/v1/addresses");
  ...
  methods: {
    async onDeleteAddress(id, index) {
      try {
        let response = await this.$axios.$delete(`/api/v1/addresses/${id}`);
    ...
    async onSetDefault(id) {
      try {
        let response = await this.$axios.$put(`/api/v1/addresses/set/default`, {
          id
        });

        if (response.success) {
          this.message = response.message;
          await this.$auth.fetchUser(); // update with new info
        }
      } catch (err) {
        this.message = err.message;
        console.log(err.message);
